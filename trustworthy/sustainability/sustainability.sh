...
# for pretrain_model in wavlm; do
# for pretrain_model in wav2vec2_0; do
# for pretrain_model in whisper_base whisper_tiny whisper_small; do
# for pretrain_model in apc; do
    # for dataset in iemocap crema_d msp-improv msp-podcast; do
    for dataset in iemocap; do
        CUDA_VISIBLE_DEVICES=0, taskset -c 90-120 python3 sustainability.py --pretrain_model $pretrain_model --dataset $dataset --learning_rate 0.0005 --downstream_model cnn --num_epochs 30 --num_layers 3 --conv_layers 2 --pooling mean --hidden_size 128
    done
done